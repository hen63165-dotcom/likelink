 ...
 * גרסה נקייה אחת — בלי כפילויות (הקובץ המקורי היה משוכפל ושבור).
 */
export function ReelsPlayer({ video, onClose, onProductClick }) {
  ...
  useEffect(() => {
    if (video?.id) trackVideoView(video.id);
  ...
  const handleProductClick = (productId) => {
    if (video?.id) trackProductClick(video.id, productId);
  ...
  const getVideoEmbedUrl = (url) => {
    ...
    if (url.startsWith("blob:") || url.startsWith("data:video") || url.match(/\.(mp4|webm|mov)$/i)) {
      return url;
    }
    return null;
  };
    ...
    <div className="relative w-full aspect-[9/16] bg-black rounded-2xl overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2 bg-white/90 backdrop-blur-sm rounded-full px-3 py-1.5 shadow-lg">
                    ...
                    <span className="text-xs font-semibold text-gray-800 whitespace-nowrap">
                      {money(details.product.price, "he")}
                    </span>
                  </div>
                  {activeTag === tag.productId && (
                    <motion.div
                      ...
                      className="mt-2 bg-white rounded-xl p-3 shadow-xl w-64"
                    >
                      {details.product.image && (
                        <img src={details.product.image} alt={details.product.title} className="w-full h-32 object-cover rounded-lg mb-2" />
                      )}
                      <p className="text-xs font-semibold text-gray-900 line-clamp-2">{details.product.title}</p>
                      <button
                        onClick={() => handleProductClick(tag.productId)}
                        className="mt-2 w-full tap text-white text-xs font-semibold py-2 rounded-lg"
                        style={{ background: "var(--accent, #B78F4F)" }}
                      >
      ...
      <div className="absolute top-4 right-4 flex gap-2">
        <button
          ...
          className="tap p-2 rounded-full bg-white/20 backdrop-blur-sm text-white"
        >
          <ShoppingBag size={18} />
        </button>
        {onClose && (
