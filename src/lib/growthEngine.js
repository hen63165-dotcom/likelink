...
export function buildRecruitPitches(prospect = {}) {
  ...
  const story = [
    ...
    `בדיוק בשבילך הקמנו את Likelink: את מדביקה לינק אחד, בוחרת סגנון, והמערכת עושה את כל השאר לבד — פוסטים אוטומטיים בעברית לכל ערוץ, מעקב קליקים, ותשלום ישר ל-PayPal.`,
    `${niche.emoji} בשבילך זה אפילו פשוט יותר: ${niche.value}.`,
    `${tier.angle}. פותחים סטודיו בחינם ב-likelink2.app — חמש דקות ואת באוויר.${link}`,
  ].join("\n");
