...
const INFLUENCER_TEMPLATES = {
  discovery: [
    "מצאתי פריט שמתאים לנתוני הקטלוג",
    "בחירה שנבחרה לפי נתוני המוצר",
    "פריט חדש מתוך הקטלוג המאומת",
    "פריט שנבחר לפי התאמה לקטגוריה",
  ],
  urgency: [
    "גילוי מוצרים לפי הקשר הזמן",
    "בדקי את הפרטים והזמינות באתר",
    "מחיר כפי שמופיע בקטלוג",
    "הבחירה הנוכחית של Luna",
  ],
  social: [
    "פריט עם נתוני ביצוע זמינים",
    "פרטי המוצר זמינים כאן",
    "מוצר שנבחר לפי אותות זמינים",
    "נתוני צפייה יוצגו רק אם נמדדו",
  ],
    fomo: [
    "לצפייה בפרטי המוצר",
    "בדקי אם המוצר מתאים לך",
    "לצפייה במוצר",
    "בחירה מתוך הקטלוג",
  ],
};
...
function generateReelFormat(product, context) {
  return {
    ...
    script: [
      ...
      { time: "3-10s", text: `${product.title} — פרטי המוצר מתוך הקטלוג` },
      { time: "10-25s", text: `מחיר הקטלוג: ₪${product.price}` },
